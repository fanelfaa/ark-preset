...
*Note: The `--no-git-tag-version` is crucial so npm doesn't attempt to commit and tag in each subdirectory.*
...
## 3. Commit and Tag
When committing `package.json` updates, the pre-commit hook (`lefthook`) might run `oxlint`. If no source files were changed, `oxlint` may exit with an error (`No files found to lint`). In this case, use `--no-verify` to bypass the hook.

```bash
git add .
git commit --no-verify -m "chore(release): bump to vX.Y.Z"
```
