...
test("switches tabs and verifies content changes", async ({ page }) => {
  ...
  await expect(page.getByText(/make changes to your account/i).first()).toBeVisible();
  ...
  await expect(page.getByText(/change your password/i).first()).toBeVisible();

  // Account panel content should be hidden
  await expect(
    page
      .locator("[data-scope='tabs']")
      .getByText(/make changes to your account/i)
      .first(),
  ).not.toBeVisible();
});
