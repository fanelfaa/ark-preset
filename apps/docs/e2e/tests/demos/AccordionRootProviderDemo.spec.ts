...
test("renders and updates external state display on toggle", async ({ page }) => {
  ...
  const trigger1 = page
    .getByRole("button", { name: "What is this demo showing?", exact: true })
    .first();
  ...
  await expect(
    page
      .locator("[data-scope='accordion']")
      .getByText(/The accordion state is managed externally/)
      .first(),
  ).toBeVisible();
  ...
  await expect(
    page
      .locator("[data-scope='accordion']")
      .getByText(/It gives you access to the accordion context/)
      .first(),
  ).not.toBeVisible();
