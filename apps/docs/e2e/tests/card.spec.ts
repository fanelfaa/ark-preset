...
test.describe("Card", () => {
  ...
  test("card: page loads without errors", async ({ page }) => {
    const errors: string[] = [];
    page.on("console", (msg) => {
      if (msg.type() === "error") errors.push(msg.text());
    });

    const response = await page.goto(componentUrl("card"));
  ...
  test("card: 'Card' section renders", async ({ page }) => {
    await page.goto(componentUrl("card"));
    await page.waitForLoadState("networkidle");
    await expect(page.getByRole("heading", { name: "Card", exact: true })).toBeVisible();
  });

  test("card: 'Usage' section renders", async ({ page }) => {
    await page.goto(componentUrl("card"));
    await page.waitForLoadState("networkidle");
    await expect(page.getByRole("heading", { name: "Usage", exact: true })).toBeVisible();
  });

  test("card: 'API Reference' section renders", async ({ page }) => {
    await page.goto(componentUrl("card"));
    await page.waitForLoadState("networkidle");
    await expect(page.getByRole("heading", { name: "API Reference", exact: true })).toBeVisible();
  });

  test("card: 'Installation' section renders", async ({ page }) => {
    await page.goto(componentUrl("card"));
    await page.waitForLoadState("networkidle");
