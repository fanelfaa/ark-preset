...
test.describe("Progress", () => {
  ...
  test("progress: page loads without errors", async ({ page }) => {
    ...
    const response = await page.goto(componentUrl("progress"));
  ...
  test("progress: 'Progress' section renders", async ({ page }) => {
    await page.goto(componentUrl("progress"));
    await page.waitForLoadState("networkidle");
    await expect(page.getByRole("heading", { name: "Progress", exact: true })).toBeVisible();
  });

  test("progress: 'Usage' section renders", async ({ page }) => {
    await page.goto(componentUrl("progress"));
    await page.waitForLoadState("networkidle");
    await expect(page.getByRole("heading", { name: "Usage", exact: true })).toBeVisible();
  });

  test("progress: 'API Reference' section renders", async ({ page }) => {
    await page.goto(componentUrl("progress"));
    await page.waitForLoadState("networkidle");
    await expect(page.getByRole("heading", { name: "API Reference", exact: true })).toBeVisible();
  });

  test("progress: 'Installation' section renders", async ({ page }) => {
    await page.goto(componentUrl("progress"));
    await page.waitForLoadState("networkidle");
