...
test.describe("Table", () => {
  ...
  test("table: page loads without errors", async ({ page }) => {
    const errors: string[] = [];
    page.on("console", (msg) => {
      if (msg.type() === "error") errors.push(msg.text());
    });

    const response = await page.goto(componentUrl("table"));
  ...
  test("table: 'Table' section renders", async ({ page }) => {
    await page.goto(componentUrl("table"));
    await page.waitForLoadState("networkidle");
    await expect(page.getByRole("heading", { name: "Table", exact: true })).toBeVisible();
  });

  test("table: 'Usage' section renders", async ({ page }) => {
    await page.goto(componentUrl("table"));
    await page.waitForLoadState("networkidle");
    await expect(page.getByRole("heading", { name: "Usage", exact: true })).toBeVisible();
  });

  test("table: 'API Reference' section renders", async ({ page }) => {
    await page.goto(componentUrl("table"));
    await page.waitForLoadState("networkidle");
    await expect(page.getByRole("heading", { name: "API Reference", exact: true })).toBeVisible();
  });

  test("table: 'Installation' section renders", async ({ page }) => {
    await page.goto(componentUrl("table"));
    await page.waitForLoadState("networkidle");
