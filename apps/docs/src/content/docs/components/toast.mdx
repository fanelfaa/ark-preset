...
export const ToastRoot: Component<ArkToast.RootProps> = (props) => {
...
import { splitProps, type Component } from "solid-js";
import { ToastRoot, ToastTitle, ToastDescription, ToastCloseTrigger, ToastActionTrigger } from "./toast.base";
  ...
  return (
    <ArkToaster class={local.class} {...others}>
      {(toast) => {
        ...
        return (
          <ToastRoot class={styles.root()}>
            <div class="grid gap-1">
              {toast().title && <ToastTitle class={styles.title()}>{toast().title}</ToastTitle>}
              {toast().description && (
                <ToastDescription class={styles.description()}>{toast().description}</ToastDescription>
              )}
            </div>
            <ToastCloseTrigger class={styles.closeTrigger()}>✕</ToastCloseTrigger>
            {toast().action && (
              <ToastActionTrigger class={styles.actionTrigger()}>{toast().action?.label}</ToastActionTrigger>
            )}
          </ToastRoot>
        );
      }}
