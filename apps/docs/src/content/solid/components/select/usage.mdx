...
```tsx
import { Select, SelectLabel, SelectTrigger, SelectContent, SelectItem } from "~/components/ui";
```

## Advanced Usage

When the composite `Select` does not provide enough control, import the raw primitive parts via `SelectBase`:
