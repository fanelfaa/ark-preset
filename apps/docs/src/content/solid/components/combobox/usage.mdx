...
```tsx
import { Combobox, ComboboxLabel, ComboboxInputTrigger, ComboboxContent, ComboboxItem } from "~/components/ui";
```

## Advanced Usage

When the composite `Combobox` does not provide enough control, import the raw primitive parts via `ComboboxBase`:

```tsx
import { ComboboxBase } from "~/components/ui";
```
