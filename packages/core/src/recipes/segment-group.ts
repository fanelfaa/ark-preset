...
export const segmentGroupVariants = tv({
  slots: {
    root: "relative inline-flex items-center p-1 h-8 rounded-md bg-muted isolation-inline",
    item: [
    ...
    itemControl: "hidden",
    indicator:
      "absolute z-0 rounded-sm bg-background shadow-sm w-(--width) h-(--height) left-(--left) transition-[left,top,width,height] duration-150 ease-out will-change-[left,top,width,height]",
  },
  variants: {
    variant: {
      solid: {
      ...
      outline: {
        ...
        item: "text-muted-foreground data-[state=checked]:text-foreground",
        indicator:
          "border border-border shadow-none will-change-[left,top,width,height]",
      },
    },
    orientation: {
      horizontal: {
